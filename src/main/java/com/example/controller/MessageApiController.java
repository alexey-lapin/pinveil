...
public class MessageApiController {
    ...
    public MessageApiController(MessageService messageService, RateLimitService rateLimitService) {
    ...
    public HttpResponse<ErrorResponse> onMessageException(MessageService.MessageException exception) {
        return switch (exception.error()) {
            case INVALID_REQUEST -> HttpResponse.status(HttpStatus.BAD_REQUEST).body(new ErrorResponse(exception.getMessage()));
            case PAYLOAD_TOO_LARGE -> HttpResponse.status(HttpStatus.valueOf(413)).body(new ErrorResponse(exception.getMessage()));
            case CAPACITY_REACHED -> HttpResponse.status(HttpStatus.SERVICE_UNAVAILABLE).body(new ErrorResponse(exception.getMessage()));
            case FORBIDDEN -> HttpResponse.status(HttpStatus.FORBIDDEN).body(new ErrorResponse("Unable to retrieve message"));
